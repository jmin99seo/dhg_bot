...
func NewClient(cfg Config) (*Client, error) {
	...
	c.RetryMax = 3
	c.RetryWaitMin = 10 * time.Microsecond
...
func updateAPIMetadata(apiKey *APIKey, resp *http.Response) {
	...
	if retryAfter, err := strconv.ParseInt(resp.Header.Get("Retry-After"), 10, 64); err == nil {
		apiKey.Reset = time.Now().Add(time.Second * time.Duration(retryAfter))
	}
}
